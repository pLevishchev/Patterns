...
class TransactionManager {
    ...
    var pendingTransactions: [Command] {
        get {
            return self._transactions.filter {
                $0.isComplete == false
            }
        }
    }
...
let account = Account(accountName: "John", balance: 1000)
...
transactionManager.addTransactions(command: Withdraw(account: account, amount: 900))
